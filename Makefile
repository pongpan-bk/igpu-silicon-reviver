...
OBJ = MyIntelGPU.o IntelFramebuffer.o MyIntelFramebuffer.o \
      ...
      MyIntelRing.o MyIntelGEMBuffer.o MyIntelGPUClient.o \
...
clean:
	...
	rm -f test_submit_vcs libmyintelvcs.dylib MyIntelVCSCommand.us.o
...
# ─── User-space Tools (Phase 9: test app + player bridge dylib) ──
# Build ด้วย user-space SDK ปกติ (ไม่ใช่ -mkernel) — ใช้กับ kext ที่โหลดอยู่:
#   make tools
#   ./test_submit_vcs clip.h264 out.nv12
#   cc player.c -L. -lmyintelvcs -framework IOKit
USER_FLAGS = -O2 -arch x86_64 -Wall  # user-space (not -mkernel)

tools: test_submit_vcs libmyintelvcs.dylib mvcs_bench
	@echo "─── Tools build complete: test_submit_vcs + libmyintelvcs.dylib + mvcs_bench ───"

MyIntelVCSCommand.us.o: MyIntelVCSCommand.cpp MyIntelVCSCommand.h
	$(CC) $(USER_FLAGS) -c -o $@ MyIntelVCSCommand.cpp

test_submit_vcs: test_submit_vcs.m MyIntelVCSCommand.us.o myintel_vcs_user.h MyIntelVCSCommand.h MyIntelVCS.h
	$(CC) $(USER_FLAGS) -o $@ test_submit_vcs.m MyIntelVCSCommand.us.o \
	  -framework IOKit -framework CoreFoundation

libmyintelvcs.dylib: myintelvcs_bridge.c myintelvcs_bridge.h MyIntelVCSCommand.us.o myintel_vcs_user.h
	$(CC) $(USER_FLAGS) -dynamiclib -install_name @rpath/libmyintelvcs.dylib \
	  -o $@ myintelvcs_bridge.c MyIntelVCSCommand.us.o \
	  -framework IOKit -framework CoreFoundation

MVCS_BENCH_LDFLAGS = -L. -lmyintelvcs -rpath "$(CURDIR)"

mvcs_bench: mvcs_bench.c libmyintelvcs.dylib
	$(CC) $(USER_FLAGS) -o $@ mvcs_bench.c $(MVCS_BENCH_LDFLAGS)
